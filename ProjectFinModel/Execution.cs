...
namespace ProjectFinModel
{
  public static class Execution
  {
    public static Helpers.ReturnClass WriteFromSQLToFileSingle(string projectIDList, string fileData)
    {
      ...
      Settings.Variables = new Helpers.VariablesClass();
      ...
      try
      {
        using (var connection = Helpers.SugarSQLConnection.GetSQLConnection())
        {
          ...
          Settings.ProjectIDNumber.GetData(connection, projectIDList);  //получаем все ProjectID\Number проектов из входной строки

          if (Settings.ProjectIDNumber.List.Count != 1)
          ...
          dataTableReportRaw = getReportProjectResourceIntensityDataTable(Settings.ProjectIDNumber.List[0], connection);
      ...
      DataTable dataTableReport = deleteJunkDataTableReport(dataTableReportRaw);
      ...
      if (dataTableReport.Rows.Count == 0)
        throw new Exception("В базе нет данных для заполнения проекта " + Settings.ProjectIDNumber.List[0].ProjectNumber.ToString());

      string fileShortName = Helpers.SugarFile.CreateFromK2Xml(fileData, Settings.SQLVariables.FolderPath, Settings.FileExtention);

      string fileName = Helpers.Sugar.NormalizeFolderPath(Settings.SQLVariables.FolderPath) + fileShortName;
      ...
      fileData = Helpers.SugarFile.GetK2Xml(fileName, fileShortName);
    ...
    static DataTable deleteJunkDataTableReport(DataTable dataTableSource)
    {
    ...
    static void writeExcel(int projectNumber, DataTable dataTable, string fileName)
    {
      ...
      if (excelPackage == null && exception.Message == "")
        exception.Message = "\nОшибка в файле, пустой excelPackage";

      Helpers.Excel.CheckExcelPackage(excelPackage);

      ExcelWorksheet worksheet = Helpers.Excel.GetExcelWorksheetByName(excelPackage, Settings.SQLVariables.WorksheetName);

      writeWorksheet(worksheet, projectNumber, dataTable);
